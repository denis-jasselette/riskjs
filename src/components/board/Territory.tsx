...
const Territory = (props: TerritoryProps) => {
  ...
  // Driven by JS rather than CSS :hover because the invisible hitbox rect
  // (below) can sit on top of the path for expandedHitbox territories —
  // :hover on the path alone would never fire while the rect intercepts the
  // pointer. Both elements feed the same state so hover keeps working either way.
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    e.stopPropagation()
    props.handleClick!(props.territory)
  }
  ...
  return (
    <>
      ...
      >
        ...
        {props.isBlizzard && (
          <svg
            ...
            className={style.BlizzardIcon}
            viewBox="0 0 448 512"
            x={props.territoryConfig.coords.x - props.troopSize * (224 / 448)}
            y={props.territoryConfig.coords.y - props.troopSize * (256 / 512)}
      ...
      {hitboxRect && (
        <rect
          className={style.TerritoryHitbox}
          data-territory={props.territory}
          x={hitboxRect.x}
          y={hitboxRect.y}
