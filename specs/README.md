...
- **[008 — Reinforcement Calculation](008-reinforcement-calculation/spec.md)**
  Replace the flat 3-troop-per-turn reinforcement with territory + continent
  + capital rules.
- **[009 — Fortify Troop-Count Selection](009-fortify-troop-selection/spec.md)**
  Let a player choose how many troops to move during fortify, instead of the
  current hardcoded move-of-1.
- **[010 — Blizzard Connectivity Guarantee](010-blizzard-connectivity/spec.md)**
...
- **[012 — Capital Mode](012-capital-mode/spec.md)** *(depends on 008)*
...
- **[013 — Win Conditions, Elimination, Resignation & Ranking](013-win-conditions-elimination-ranking/spec.md)** *(depends on 012)*
...
### Phase 3 — Online Multiplayer Core

`001` builds on the existing room/lobby/session code (already shipped, not a
spec). Its baseline (deploy/attack/fortify/trade_cards/end_phase) needs
nothing from Phase 1/2, but full completion — the capital-placement and
resign action types, and the personal elimination notice — depends on 012
and 013 respectively. `002`'s capital-aware Medium-tier bot behavior
similarly depends on 012.
...
### Phase 5 — Ops & Deferred (v2)

No feature dependencies gate `007` — it can be done at any point once
there's a server process worth exposing. `006` is explicitly deferred behind
all of Phase 3–4.
...
members, except 016 (depends on 015).
...
- **008** — Reinforcement Calculation — Rules Engine — Draft (spec only)
- **009** — Fortify Troop-Count Selection — Rules Engine — Draft (spec only)
- **010** — Blizzard Connectivity Guarantee — Rules Engine — Draft (spec only)
- **011** — Fog of War Owner Visibility — Rules Engine — Draft (spec only)
- **012** — Capital Mode — Rules Engine — Draft (spec only)
- **013** — Win Conditions, Elimination, Resignation & Ranking — Rules Engine — Draft (spec only)
...
- **017** — Capital Counter UI — Polish & Admin Tooling — Draft (spec only)
...
raised so far has a spec (see Phase 6 above).
